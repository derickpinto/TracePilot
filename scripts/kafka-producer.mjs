 ...
 * ─────────────────────────────────────────────────────────────────
 * Publishes realistic dummy workflow events directly to Kafka,
 * testing the full consumer pipeline inside the API container.
 *
 * Prerequisites:
 *   pnpm install          (installs kafkajs at workspace root)
 *   docker compose -f docker-compose.dev.yml up --build
 *
 * Usage:
 *   node scripts/kafka-producer.mjs
 *   node scripts/kafka-producer.mjs --once      # one batch then exit
 *   node scripts/kafka-producer.mjs --speed 2   # 2× faster (default: 1)
 *
 * Connects to Kafka's external listener at localhost:9094.
 * ─────────────────────────────────────────────────────────────────
 */
